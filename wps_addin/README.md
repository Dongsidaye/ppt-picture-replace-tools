...
- 对象筛选：功能区「筛选」菜单只改变当前幻灯片、母版或版式视图中的对象选区，支持「选定」（当前页全选）、反选、相同类型/字号/宽度/高度/颜色，以及所有线条、文字和组合对象。
- 图层管理：打开插件自带的 WPS 图层管理窗格，可点击行或“定位”跳转并选中正文对象，逐层控制显示/隐藏和锁定/解锁；当前 WPS 若未提供可验证的原生 `Shape.Locked`，锁定会保存为对象 `Tags` 中的插件标记，并在界面说明其不能阻止 WPS 画布直接编辑。
