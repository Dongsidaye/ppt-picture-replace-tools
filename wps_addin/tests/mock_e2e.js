...
async function main() {
  ...
  const designIconIds = { DesignStyleBrushButton: "icon_design_style.png", DesignTextToolsButton: "icon_design_text.png", DesignLayoutToolsButton: "icon_design_layout.png", DesignCleanupToolsButton: "icon_design_cleanup.png", DesignExportToolsButton: "icon_design_export.png", DesignColorToolsButton: "icon_design_color.png", DesignPhotoshopToolsButton: "icon_design_photoshop.png" };
  check("design tool groups use dedicated ribbon icons", Object.keys(designIconIds).every(function (id) { return global.OnGetToolsImage({ Id: id }) === designIconIds[id] && fs.existsSync(path.join(__dirname, "..", designIconIds[id])); }) && global.OnGetToolsImage({ Id: "UnknownButton" }) === "icon_filter.png" && global.OnGetToolsImage() === "icon_filter.png", "design group icons");
  ...
  check("ribbon visibly exposes the current version", /id="AddonVersion"[^>]*label="v2\.1\.10"/.test(ribbonXml), "AddonVersion");
  ...
  check("background preload waits until the picture panel is used", /let panelInventoryPanelUsed = false;/.test(src) && /function OpenPicturePanel\(\) \{\s*panelInventoryPanelUsed = true;/.test(src) && /if \(!panelInventoryPanelUsed\) return;/.test(src), "panel-used gate");
  ...
  check("batch buttons re-list the current context before applying", /function layerFreshList\(matchFn\)/.test(taskpaneHtml) && /layerFreshList\(layerTypeKeysOf\(seeds\)\) \|\| seeds/.test(taskpaneHtml) && /layerFreshList\(matchLocked\)/.test(taskpaneHtml), "fresh snapshot before batch");
  ...
  global.XMLHttpRequest = function () { return new MockXHR({ name: "picture-replace-tools-wps", version: "2.1.11" }, 200); };
  const up = await W.checkForUpdates();
  check("update check detects newer", up.ok === true && up.hasUpdate === true && up.latest === "2.1.11", JSON.stringify(up));
  check("update check builds download url", /releases\/download\/v2\.1\.11\/PictureReplaceTools-WPS-2.1.11\.exe$/.test(up.downloadUrl || ""), up.downloadUrl || "");
  ...
  global.__mockXhrRoute = function (url) {
    ...
    return { status: 200, responseText: "", responseURL: "https://github.com/Dongsidaye/ppt-picture-replace-tools/releases/tag/v2.1.11" };
    }
    return null;
  };
  const upFallback = await W.checkForUpdates();
  check("update check falls back to release tag", upFallback.ok === true && upFallback.hasUpdate === true && upFallback.latest === "2.1.11", JSON.stringify(upFallback));
